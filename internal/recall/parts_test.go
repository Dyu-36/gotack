...
func TestExtractPartsText(t *testing.T) {
	cases := []struct {
		...
		{
			name: "reasoning and text combined",
			build: func(t *testing.T) string {
				return partsArray(
			...
			want: []string{"consider the rollback", "rolled back the release"},
		},
		{
			name: "tool call and tool result",
			build: func(t *testing.T) string {
				return partsArray(
			...
			want: []string{"exec", "kubectl get pods", "three pods running"},
		},
		{
			name: "shell command with output and finish message",
			build: func(t *testing.T) string {
				return partsArray(
			...
			want: []string{"pnpm build", "built ok", "provider rate limited"},
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := extractPartsText(tc.build(t))
			offset := 0
...
func TestExtractPartsTextDefensive(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := extractPartsText(tc.raw); got != "" {
