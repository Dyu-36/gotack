...
| office_seed.go, context_seed.go, guard_seed.go, schedule_host.go, settings_crush.go | package main helpers that are not bound methods; they take `*App` only for config and resource seeding |
| internal/ | Desktop-side implementation, one package per role: appconfig, attachments, changes, contextseed, crushapi, engine, enginelink, guard, logging, mcp, office, officecli, permission, recall, schedule, session, terminal, uievents, workspace, zalo |
